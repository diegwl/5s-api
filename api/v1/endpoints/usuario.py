...
# GET Usuario
@router.get('/{usuario_id}', response_model=UsuarioSchemaTarefas, status_code=status.HTTP_200_OK)
async def get_usuario(usuario_id: int, db: AsyncSession = Depends(get_session)):
    async with db as session:
        query = select(UsuarioModel).filter(UsuarioModel.id == usuario_id)
        result = await session.execute(query.options(joinedload(UsuarioModel.tarefas)))
        
        usuario: UsuarioSchemaTarefas = result.scalars().unique().one_or_none()
